...
save_dir = "./models/D_hyper_params"
...
for lr in [0.001, 0.005, 0.01]:  # Example learning rates to explore
    for wd in [0.0001, 0.001]:  # Example weight decay values
        for dr   in [0.25, 0.5]:
            train_accuracies = []
            test_accuracies = []
            train_losses = []
            test_losses = []
            model = Perceptron().to(device)
            model.dropout = nn.Dropout(dr)  # Set dropout rate
            optimizer = optim.Adam(model.parameters(), lr=lr, weight_decay=wd)  # Set optimizer
            scheduler = ReduceLROnPlateau(optimizer, mode="min", factor=0.2, patience=10)
            save_result_path = f"{results_dir}/lr{lr}_wd{wd}_dr{dr}"

            model.eval()
            initial_predictions = []
            initial_targets = []

            print(f"learn rate: {lr}, weight decay: {wd}, dropout rate: {dr}")
            ...
            for epoch in range(num_epochs):
                ...
                weight_path = weight_dir+f"/lr{lr}_wd{wd}_dr{dr}/epoch_{epoch}.pth"

                if not os.path.exists(weight_dir+f"/lr{lr}_wd{wd}_dr{dr}/"):
                    os.makedirs(weight_dir+f"/lr{lr}_wd{wd}_dr{dr}/")
